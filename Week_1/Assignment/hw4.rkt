...
(define (sequence low high stride)
  (cond [(> low high) null]
        [(= low high) (list low)]
        [#t (append (list low) (sequence (+ low stride) high stride))])) ; append is O(n)
                                                                         ; so this implementation is quadratic, cons better instead
...
; unnecessarily complicated, better alternative
(define (stream-for-n-steps-better s n)
  (if (= n 0)
      null
      (let ([next (s)])
        (cons (car next) (stream-for-n-steps-better (cdr next) (- n 1))))))
...
(define funny-number-stream-better
...
; error because each iteration of the inner function f resets the pos variable to 0
; since pos defined within the body of f
; instead, define pos outside of f but within cached-assoc to access it through the closure of f.
